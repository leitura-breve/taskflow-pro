...
def build_catalog(books_full):
    ...
    def _cover(c, doc):
        ...
        c.setCharSpace(-8); c.drawString(MARGIN_L - 0.3*cm, PAGE_H - 11*cm, "53"); c.setCharSpace(0)
        # kicker
        ...
        c.drawString(MARGIN_L, PAGE_H - 17.6*cm, "Catálogo dos 53 volumes lançados — bestsellers, clássicos")
        ...
        c.line(MARGIN_L, 2.3*cm, PAGE_W - MARGIN_R, 2.3*cm)
        c.setFont("Helvetica-Bold", 7.5); c.setFillColor(muted); c.setCharSpace(2.5)
        c.drawString(MARGIN_L, 1.7*cm, "CATÁLOGO COMPLETO  ·  COLEÇÃO LEITURA BREVE  ·  VOL. 01–53")
        c.drawRightString(PAGE_W - MARGIN_R, 1.7*cm, "FORMATO A4"); c.setCharSpace(0)
    ...
    story.append(HLine(fw, thickness=0.6, color=ink, space_after=12))
    story.append(Paragraph(
        "Coleção dos 53 volumes lançados pela <b>Leitura Breve</b> — resumos editoriais "
    ...
    story.append(Paragraph("Os 53 volumes em três páginas.", section))
    story.append(HLine(fw, thickness=0.6, color=ink, space_after=12))
...
def load_more_books():
    """Carrega os Vols. 08-18 + 19-38 + 39-53."""
    result = []
    try:
        import books_data
    ...
    try:
        import books_data_3
        result.extend(books_data_3.MORE_BOOKS_3)
    except ImportError:
        pass
    return result
